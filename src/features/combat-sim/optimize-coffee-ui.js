...
/**
 * Generate all combinations of up to MAX_DRINK_SLOTS groups (including the empty combo).
 * @param {Array} groups
 * @returns {Array<Array>}
 */
function generateCombos(groups) {
    const combos = [[]];
    const recurse = (start, current) => {
        if (current.length > 0) combos.push([...current]);
        if (current.length >= MAX_DRINK_SLOTS) return;
...
class OptimizeCoffeeUI {
    ...
    async _onRun() {
        ...
        const combos = generateCombos(groups);
